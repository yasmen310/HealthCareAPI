...
class AssistantSerializer(serializers.ModelSerializer):
    ...
    def get_assistantIMG(self, obj):
        request = self.context.get('request')
        if obj.assistantIMG:
          return request.build_absolute_uri(obj.assistantIMG.url)
        return None
...
class HospitalSerializer(serializers.ModelSerializer):
    ...
    def get_clinics(self, obj):
        ...
        # image 
    def get_hospitalIMG(self, obj):
        request = self.context.get('request')
        if obj.hospitalIMG:
            return request.build_absolute_uri(obj.hospitalIMG.url)
        return None
...
class DoctorSerializer(serializers.ModelSerializer):
    ...
    def get_doctorIMG(self, obj):
        request = self.context.get('request')
        if obj.doctorIMG:
            return request.build_absolute_uri(obj.doctorIMG.url)
        return None
...
class AppointmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Appointment
        fields = ['appointment_id', 'appointment_date', 'appointment_time', 'doctor_name', 'patient_name', 'specialization_name', 'clinic_name', 'doctor', 'patient', 'specialization', 'clinic']
  
    def get_specialization_name(self, obj):
        specialization = obj.specialization
        return specialization.departmentName if specialization else None
    def get_doctor_name(self, obj):
        return obj.doctor.doctorName
    def get_patient_name(self, obj):
        return obj.patient.patientName
    def get_clinic_name(self, obj):
        return obj.clinic.clinicName if obj.clinic else None
...
class TransformSerializer(serializers.ModelSerializer):
     ...
     class Meta:
        model = Transform
        fields = ['id', 'name', 'age', 'reportIMG', 'reportText', 'hospital_name', 'specialization_name', 'timestamp', 'hospital', 'specialization']
     ...
     def get_reportIMG (self, obj):
        request = self.context.get('request')
        if obj.reportIMG :
            return request.build_absolute_uri(obj.reportIMG.url)
        return None
...
class PatientSerializer(serializers.ModelSerializer):
    ...
    def get_patientIMG (self, obj):
        request = self.context.get('request')
        if obj.patientIMG :
            return request.build_absolute_uri(obj.patientIMG.url)
        return None
